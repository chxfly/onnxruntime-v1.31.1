...
if [ $RunTestCsharp = "true" ]; then
  ...
  if [ $PACKAGENAME = "Microsoft.ML.OnnxRuntime.Gpu" ]; then
    export TESTONGPU=ON
    # dotnet test -p:DefineConstants=USE_CUDA $BUILD_SOURCESDIRECTORY/csharp/test/Microsoft.ML.OnnxRuntime.EndToEndTests/Microsoft.ML.OnnxRuntime.EndToEndTests.csproj --no-restore --verbosity detailed --filter TestCUDAProviderOptions
