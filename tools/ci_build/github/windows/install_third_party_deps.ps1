...
[string[]]$cmake_extra_args="-DCMAKE_CXX_STANDARD=17 `"-DCMAKE_CXX_FLAGS=$compile_flags /EHsc`" ", "`"-DCMAKE_C_FLAGS=$compile_flags`",", "`"-DCMAKE_CXX_COMPILER_LAUNCHER=ccache`",", "`"-DCMAKE_C_COMPILER_LAUNCHER=ccache`",","--compile-no-warning-as-error", "--fresh", "-Wno-dev"
if($cpu_arch -eq 'x86'){
