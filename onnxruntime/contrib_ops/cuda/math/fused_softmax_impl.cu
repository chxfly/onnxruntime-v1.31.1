...
#include "fused_softmax_impl.cuh"
...
__global__ void fused_softmax_warp_forward(output_t* dst, const input_t* src, const bool *mask, int batch_size, int stride, int element_count, int pad_batches) {
  ...
  for (int i = 0; i < WARP_BATCH; ++i) {
    ...
    for (int it = 0; it < WARP_ITERATIONS; ++it) {
      ...
      if (element_index < batch_element_count && mask[itr_idx] != true) {
        // if (mask[itr_idx] != true) {
        //   elements[i][it] = (acc_t)src[itr_idx];
        // } else {
        //   elements[i][it] = -10000.0;
        // }
        elements[i][it] = src[itr_idx];
      } else {
...
void dispatch_fused_softmax_forward(cudaStream_t stream, output_t* dst, const input_t* src, const bool* mask, 
      int softmax_elements, int softmax_elements_stride, int batches, int attn_heads, int pad_batches) {
  if (softmax_elements == 0) {
    ...
    // int blocks = (batch_count + batches_per_block - 1) / batches_per_block;
	  ...
	  ORT_ENFORCE(seq_len % batches_per_block == 0, "seq_len % batches_per_block != 0,");
    dim3 blocks(seq_len / batches_per_block, attn_heads, batches);
    dim3 threads(warp_size, warps_per_block, 1);
    // Launch code would be more elegant if C++ supported FOR CONSTEXPR
    switch (log2_elements) {
      case 0:  // 1
        fused_softmax_warp_forward<input_t, output_t, acc_t, 0, is_log_softmax>
            <<<blocks, threads, 0, stream>>>(dst, src, mask, batch_count, softmax_elements_stride, softmax_elements, pad_batches);
        break;
      case 1:  // 2
        fused_softmax_warp_forward<input_t, output_t, acc_t, 1, is_log_softmax>
            <<<blocks, threads, 0, stream>>>(dst, src, mask, batch_count, softmax_elements_stride, softmax_elements, pad_batches);
        break;
      case 2:  // 4
        fused_softmax_warp_forward<input_t, output_t, acc_t, 2, is_log_softmax>
            <<<blocks, threads, 0, stream>>>(dst, src, mask, batch_count, softmax_elements_stride, softmax_elements, pad_batches);
        break;
      case 3:  // 8
        fused_softmax_warp_forward<input_t, output_t, acc_t, 3, is_log_softmax>
            <<<blocks, threads, 0, stream>>>(dst, src, mask, batch_count, softmax_elements_stride, softmax_elements, pad_batches);
        break;
      case 4:  // 16
        fused_softmax_warp_forward<input_t, output_t, acc_t, 4, is_log_softmax>
            <<<blocks, threads, 0, stream>>>(dst, src, mask, batch_count, softmax_elements_stride, softmax_elements, pad_batches);
        break;
      case 5:  // 32
        fused_softmax_warp_forward<input_t, output_t, acc_t, 5, is_log_softmax>
            <<<blocks, threads, 0, stream>>>(dst, src, mask, batch_count, softmax_elements_stride, softmax_elements, pad_batches);
        break;
      case 6:  // 64
        fused_softmax_warp_forward<input_t, output_t, acc_t, 6, is_log_softmax>
            <<<blocks, threads, 0, stream>>>(dst, src, mask, batch_count, softmax_elements_stride, softmax_elements, pad_batches);
        break;
      case 7:  // 128
        fused_softmax_warp_forward<input_t, output_t, acc_t, 7, is_log_softmax>
            <<<blocks, threads, 0, stream>>>(dst, src, mask, batch_count, softmax_elements_stride, softmax_elements, pad_batches);
        break;
      case 8:  // 256
        fused_softmax_warp_forward<input_t, output_t, acc_t, 8, is_log_softmax>
            <<<blocks, threads, 0, stream>>>(dst, src, mask, batch_count, softmax_elements_stride, softmax_elements, pad_batches);
        break;
      case 9:  // 512
        fused_softmax_warp_forward<input_t, output_t, acc_t, 9, is_log_softmax>
            <<<blocks, threads, 0, stream>>>(dst, src, mask, batch_count, softmax_elements_stride, softmax_elements, pad_batches);
        break;
      case 10:  // 1024
        fused_softmax_warp_forward<input_t, output_t, acc_t, 10, is_log_softmax>
            <<<blocks, threads, 0, stream>>>(dst, src, mask, batch_count, softmax_elements_stride, softmax_elements, pad_batches);
        break;
      default:
        break;
...
template void dispatch_fused_softmax_forward<input_t, output_t, acc_t, false>(cudaStream_t stream, output_t * dst, const input_t* src, const bool* mask, int softmax_elements, int softmax_elements_stride, int batches, int attn_heads, int first_dim); \
template void dispatch_fused_softmax_forward<input_t, output_t, acc_t, true>(cudaStream_t stream, output_t * dst, const input_t* src, const bool* mask, int softmax_elements, int softmax_elements_stride, int batches, int attn_heads, int first_dim);
