...
Status AddToFeeds(const IExecutionProvider* execution_provider,
  ...
  for (auto& input : inputs) {
    if (input.IsAllocated()) {
      //bugbug: support different types
      total_bytes += input.Get<Tensor>().Shape().Size() * sizeof(int32_t);
    }
  }
  ...
  for (auto& input : inputs) {
    if (input.IsAllocated()) {
      ...
      //bugbug: support different types
      const size_t bytes = sizeof(int32_t) * tensor.Shape().Size();
  ...
  for (auto& input : inputs) {
    if (input.IsAllocated()) {
      ...
      const size_t bytes = sizeof(int32_t) * shape.Size();
      MLDataType dataType = tensor.DataType();
