...
#ifdef ENABLE_TRAINING
      .def("add_yield_transformer", [](PyInferenceSession* sess,
                                       const std::vector<std::string>& input_names = {},
                                       const std::vector<std::string>& output_names = {}) -> void {
        auto rule_transformer_L1 = std::make_unique<RuleBasedGraphTransformer>("YieldIORewriter");
        rule_transformer_L1->Register(std::make_unique<YieldIORewriter>(input_names, output_names));
        Status status = sess->GetSessionHandle()->RegisterGraphTransformer(std::move(rule_transformer_L1), TransformerLevel::Level1);
        if (!status.IsOK()){
          throw std::runtime_error("Error in applying yield transform: " + status.ErrorMessage());
        }
      })
#endif
